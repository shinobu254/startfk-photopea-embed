...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as photopea_tab:
        ...
        with gr.Row():
            with gr.Column():
                gr.HTML(
                    """<b>Controlnet extension not found!</b> Either <a href="https://github.com/Mikubill/" target="_blank">install it</a>, or activate it under Settings.""",
        ...
        # The getAndSendImageTostartfkTab in photopea-bindings.js takes the following parameters:
        #  startfkTab: the name of the tab. Used to find the gallery via DOM queries.
        #  sendToControlnet: if true, tries to send it to a specific ControlNet widget, otherwise, sends to the native startfk widget.
        #  controlnetModelIndex: the index of the desired controlnet model tab.
        send_t2i_cn.click(
            None,
            select_target_index,
            None,
            _js="(i) => {getAndSendImageToStartfkTab('txt2img', true, i)}",
        )
        send_extras.click(
            None,
            select_target_index,
            None,
            _js="(i) => {getAndSendImageToStartfkTab('extras', false, i)}",
        )
        send_i2i.click(
            None,
            select_target_index,
            None,
            _js="(i) => {getAndSendImageToStartfkTab('img2img', false, i)}",
        )
        send_i2i_cn.click(
            None,
            select_target_index,
            None,
            _js="(i) => {getAndSendImageToStartfkTab('img2img', true, i)}",
        )
        send_selection_inpaint.click(fn=None, _js="sendImageWithMaskSelectionToStartfk")
...
# Actually hooks up the tab to the startfk tabs.
