...
// Creates a button in one of the startfk galleries that will get the currently selected image in the 
// gallery.
// `queryId`: the id for the querySelector to search for the specific gallery list of buttons.
// `gallery`: the gallery div itself (cached by startfk).
...
function goToImg2ImgInpaintUpload(onFinished) {
    ...
    waitForStartfkUpdate(img2imgdiv).then(() => {
...
// Gets the currently selected image in a startfk gallery and opens it in Photopea.
...
// actually send it to the startfk.
function getAndSendImageToStartfkTab(startfkTab, sendToControlnet, imageWidgetIndex) {
    ...
    postMessageToPhotopea(saveMessage)
        .then((resultArray) => {
            ...
            // base64 string, then to blob, so it can be sent to a specific image widget in startfk.
...
function sendImageToStartfk(startfkTab, sendToControlNet, controlnetModelIndex, blob) {
    ...
    if (sendToControlNet) {
        ...
        const tabId = startfkTab === "txt2img"
        ...
        const imageInput = gradioApp().getElementById(`mode_${startfkTab}`).querySelector("input[type='file']");
...
// from selection.
function sendImageWithMaskSelectionToStartfk() {
...
function setImageOnControlNetInput(controlNetDiv, controlNetModelIndex, file) {
    ...
    waitForStartfkUpdate(controlNetDiv).then(() => {
        // When more than one Controlnet model is enabled in the startfk settings, there will be a
...
async function waitForStartfkUpdate(divToWatch) {
